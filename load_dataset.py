...
# 定义保存路径
output_dir = r"D:\科研24.9\我的文件\output_images"  # 自定义保存目录
os.makedirs(output_dir, exist_ok=True)
image_path = os.path.join(output_dir, f"image_{example['session_id']}.jpg")
...
def prepare_feature_vocabularies(dataset_split):
    ...
    if not dataset_split or not dataset_split[0].get("turns"):
        raise ValueError("No turns data available in the dataset split")
    return {
        "domain": dataset_split.features["turns"].feature["domain"],
        "query_category": dataset_split.features["turns"].feature["query_category"],
        "dynamism": dataset_split.features["turns"].feature["dynamism"],
...
print(dataset[split_to_use][0])
print(dataset[split_to_use][0]["turns"][0])  # 应输出 {'interaction_id': ..., 'domain': 0, ...}
